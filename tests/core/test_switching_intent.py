...
async def test_the_encoders_are_canonical_and_omit_empty_values_and_families(adapter_client):
    ...
    async with session() as db:
        lag_document = {"lag": await freeze_snapshot(db, device_id, "lag")}
        switchport_document = {"switchport": await freeze_snapshot(db, device_id, "switchport")}
...
def test_obsolete_direct_nso_switching_paths_are_absent():
    ...
    switching_source = (repository / "nso_adapter/core/switching_intent.py").read_text(encoding="utf-8")
