...
"""Tests for sync_state drift checker."""
...
def test_sync_state_unknown_when_no_netbox():
    assert compute_sync_state("uplink", None) == SyncState.unknown


def test_sync_state_imported_when_equal():
    assert compute_sync_state("uplink", "uplink") == SyncState.imported


def test_sync_state_changed_when_different():
    assert compute_sync_state("new-desc", "old-desc") == SyncState.changed


def test_sync_state_unknown_for_none_nso_and_none_nb():
    assert compute_sync_state(None, None) == SyncState.unknown


def test_sync_state_changed_for_none_nso_with_existing_nb():
    assert compute_sync_state(None, "old") == SyncState.changed
