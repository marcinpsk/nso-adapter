...
def _fake_nso(devices=_NSO_DEVICES):
    # The NSO RESTCONF client is a true external boundary (HTTP to NSO we can't run in a
    # unit test). spec=NsoClient bounds the fake to the real interface so a renamed/removed
    # client method can't be silently fabricated; only the HTTP payload is canned.
...
async def test_list_devices_enriched_fields_present(adapter_client_with_nso):
    """Response includes all nullable fields; no key omitted."""
    with patch("nso_adapter.api.nso_instances.get_nso_client", return_value=_fake_nso()):
...
async def test_list_devices_cisco_ned_derives_platform(adapter_client_with_nso):
...
async def test_list_devices_null_fields_for_minimal_payload(adapter_client_with_nso):
...
async def test_list_devices_onboarded_cross_reference(adapter_client_with_nso):
...
async def test_list_devices_sorted_by_name(adapter_client_with_nso):
...
async def test_list_devices_onboarded_no_netbox_match(adapter_client_with_nso):
    ...
    with patch("nso_adapter.api.nso_instances.get_nso_client", return_value=_fake_nso(nso_devices)):
        resp = await adapter_client_with_nso.get(
...
async def test_list_devices_cross_instance_isolation(adapter_client_with_nso):
