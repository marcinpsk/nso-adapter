...
async def _get_intents(device_id: int):
    ...
    async for db in get_session():
        rows = (
            await db.execute(select(L2SapIntent).where(L2SapIntent.device_id == device_id))
        ).scalars().all()
    ...
    body = {
        "saps": [
            {"service_name": "TL", "service_type": "epipe", "sap_id": "lag-60:3999",
             "port": "lag-60", "outer_tag": 3999},
            {"service_name": "701", "service_type": "vpls", "sap_id": "1/1/c31/3:701",
             "port": "1/1/c31/3", "outer_tag": 701},
        ]
    }
...
async def test_put_l2_sap_intent_full_replace_prunes(adapter_client):
    ...
    resp = await adapter_client.put(
        f"/api/v1/devices/{device_id}/l2-sap-intent", json=second, headers=AUTH
    )
...
async def test_put_l2_sap_intent_unknown_device_404(adapter_client):
    resp = await adapter_client.put(
        "/api/v1/devices/999999/l2-sap-intent", json={"saps": []}, headers=AUTH
    )
    assert resp.status_code == 404
